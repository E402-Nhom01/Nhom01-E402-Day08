...
**Grading JSONL:** `artifacts/eval/grading_run.jsonl` — 3 dòng valid, tất cả đạt:

| ID | top1_doc_id | contains_expected | hits_forbidden | top1_doc_matches | Điểm |
|----|-------------|-------------------|----------------|------------------|------|
| `gq_d10_01` (refund 7d) | `policy_refund_v4` | true | false | n/a | 4/4 |
| `gq_d10_02` (P1 SLA 4h) | `sla_p1_2026` | true | false | n/a | 3/3 |
| `gq_d10_03` (HR 12d) | `hr_leave_policy` | true | false | **true** | 3/3 |

Sanity check GV (`instructor_quick_check.py --grading ...`): **3/3 MERIT_CHECK OK**.

**Hạng đề xuất: Merit** (SCORING dòng 17) — vì `gq_d10_03` đạt đủ `contains_expected=true`, `hits_forbidden=false`, `top1_doc_matches=true` → đây chính là evidence cho `q_leave_version` mà nhóm từng ghi "chưa có".

---

## 8. Rủi ro còn lại & việc chưa làm

- **Publish-boundary freshness:** mới đo ở ingest; thêm metric `run_timestamp - max(cleaned_exported_at)` để bắt trường hợp ingest tươi nhưng publish chậm. Distinction (b) / Bonus (+1).
- **GE/pydantic:** expectation hiện tại là dataclass tự viết — nếu tích hợp Great Expectations sẽ đạt Distinction (a) / Bonus (+2).
- **Cutoff versioning hard-code:** `FAR_FUTURE_CUTOFF = "2028-01-01"` trong `cleaning_rules.py` và `hr_leave_min_effective_date: "2026-01-01"` trong yaml đang tách 2 chỗ — move vào env hoặc đọc từ yaml để tránh drift (Distinction (d)).
